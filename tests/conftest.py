...
``wallet_helper/`` to collect docstring examples. Two surfaces,
:mod:`wallet_helper.cli_click` and :mod:`wallet_helper.api`, raise ``SystemExit``
on import when their optional dependency is missing. On a core install we drop
them from collection so the suite stays green; when the extras are installed
(CI, or a full local env) they are collected and their doctests run.
