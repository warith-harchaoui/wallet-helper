...
:mod:`wallet_helper.cli_click`, :mod:`wallet_helper.api`,
:mod:`wallet_helper.mcp_server` — raise ``SystemExit`` on import when their
optional dependency is missing. On a core-only install we drop them from
collection so the suite stays green; when the extras are installed (CI, or a
full local env) they are collected and their doctests run.
...
_OPTIONAL = [
    ...
    ("wallet_helper/mcp_server.py", "mcp"),
]
